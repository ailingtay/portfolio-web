...
(() => {
  ...
  const items = [...document.querySelectorAll('.motion')].map((el, index) => {
    ...
    return {el, video, index, button: el.querySelector('.play-control'), status: el.querySelector('.media-error'),
      visible: false, near: false, warm: false, userPaused: false, manual: false,
  ...
  async function download(resource) {
    ...
    try {
      // Local file previews cannot fetch() media because file pages have a null
      // origin. Video elements can load the same relative paths directly.
      if (location.protocol === 'file:') {
        resource.blobUrl = resource.src;
        resource.bytes = 0;
